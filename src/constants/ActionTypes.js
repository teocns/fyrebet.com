import keyMirror from "keymirror";
import JackpotRouletteActionTypes from "./ActionTypes/JackpotRoulette";
import SessionActionTypes from "./ActionTypes/Session";
import ChatActionTypes from "./ActionTypes/Chat";
import UserActionTypes from "./ActionTypes/User";
import RatesActionTypes from "./ActionTypes/Rates";

const ActionTypes = {
  JackpotRoulette: JackpotRouletteActionTypes,
  Session: SessionActionTypes,
  Chat: ChatActionTypes,
  User: UserActionTypes,
  Rates: RatesActionTypes,
};

const _ActionTypes = keyMirror({
  // SOCKET
  API_ERROR: null,
  API_SUCCESS: null,
  // RATES_UPDATED: null,
  // RATES_SHOULD_DISPLAY_USD: null,
  SOCKET_CONNECTED: null,
  SESSION_ID_RECEIVED: null,
  SOCKET_AUTHENTICATION_TOKEN_SENT: null,
  SOCKET_MESSAGE_SENT: null,
  SOCKET_MESSAGE_RECEIVED: null,
  SOCKET_CHAT_MESSAGE_RECEIVED: null,
  SOCKET_CHAT_MESSAGE_SENT: null,
  CHAT_MESSAGE_SENT: null,
  CHAT_MESSAGE_RECEIVED: null,
  CHAT_STATUS_RECEIVED: null,
  CHAT_THREAD_DATA_RECEIVED: null,
  CHAT_ROOM_CHANGE: null,
  CHAT_THREAD_CLOSE: null,
  CHAT_HISTORY_RECEIVED: null,
  CHAT_SEARCH_QUERY: null,
  CHAT_SEARCH_QUERY_API_CALL_EXECUTED: null,
  CHAT_SEARCH_RESULTS_CHANGED: null,
  CHAT_MODE_CHANGE: null,
  CHAT_PREPARE_SEARCH_API_CALL: null,
  CHAT_PUBLIC_ROOMS_RECEIVED: null,
  CHAT_INITIALIZED: null,
  USER_DATA_SEARCH_QUERY: null,
  USER_DATA_QUERY_API_CALL: null,
  USER_DATA_RESULTS_CHANGED: null,
  UI_ON_LOGIN_MODAL_TOGGLED: null,
  UI_ON_LOGIN_MODAL_OPENED: null,
  UI_ON_LOGIN_MODAL_CLOSED: null,
  UI_ON_SIDEBAR_TOGGLE: null,
  UI_USER_PROFILE_MODAL_TOGGLE: null,
  UI_SIDEBAR_CONTENT_CHANGING: null,
  UI_SIDEBAR_CONTENT_CHANGED: null,
  UI_SHOW_SNACKBAR: null,
  UI_GOTO_VIEW: null,
  UI_TOGGLE_PRINT_RATES_USD: null,
  UI_ERROR: null,
  UI_UNFOCUS_FROM_APP_DRAWER: null,
  UI_FOCUS_ON_APP_DRAWER: null,
  UI_CHANGE_APP_DRAWER_VIEW: null,
  LANGUAGE_CHANGE: null,
  SESSION_USER_DATA_RECEIVED: null,
  SESSION_USER_AVATAR_CHANGED: null,
  SESSION_USER_UPDATED: null,
  SESSION_CHAT_MESSAGE_RECEIVED: null,
  SESSION_USER_LOGOUT: null,
  SESSION_AUTHENTICATION_TOKEN_RECEIVED: null,
  SESSION_INITIAL_STATUS_RECEIVED: null,
  AUTHENTICATION_FAILED: null,
  // BALANCE_ACTIVE_CURRENCY_CHANGED: null,
  // BALANCE_UPDATED: null,
  // USER_BALANCE_CHANGED: null,
  GAME_FORTUNE_WHEEL_USER_BET_X2: null,
  GAME_FORTUNE_WHEEL_USER_BET_X3: null,
  GAME_FORTUNE_WHEEL_USER_BET_X5: null,
  GAME_FORTUNE_WHEEL_USER_BET_X50: null,
  GAME_FORTUNE_WHEEL_ROUND_BEGIN: null,
  GAME_FORTUNE_WHEEL_ROUND_DRAW: null,
  GAME_FORTUNE_WHEEL_USER_BET: null,
  GAME_FORTUNE_WHEEL_BETS_CLOSED: null,
  GAME_FORTUNE_WHEEL_STATUS: null,
  GAME_FORTUNE_WHEEL_ROOM_JOIN: null,
  GAME_FORTUNE_WHEEL_ROOM_LEAVE: null,
  NOTIFY_MESSAGE_RECEIVED: null,
  PLAY_SOUND_EFFECT: null,
  DUEL_LOBBY_DATA: null,
  DUEL_LOBBY_USER_JOINED: null,
  DUEL_LOBBY_GAME_STARTED: null,
  GAME_JACKPOT_ROULETTE_ROUND_DRAW: null,
  GAME_JACKPOT_ROULETTE_ROUND_NEW: null,
  GAME_JACKPOT_ROULETTE_ROUND_BET: null,
  GAME_JACKPOT_ROULETTE_THREADS_SYNC_RECEIVED: null, // All threads brief
  GAME_JACKPOT_ROULETTE_THREAD_SYNC_RECEIVED: null, // Single thread
  GAME_JACKPOT_ROULETTE_THREADS_BRIEF_RECEIVED: null,
  GAME_JACKPOT_ROULETTE_THREAD_CHANGE: null,
});

export default ActionTypes;
